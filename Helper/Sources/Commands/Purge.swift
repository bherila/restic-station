...
struct PurgePreview: AsyncParsableCommand, JSONRenderable {
    ...
    static func validate(
        ...
        case .secretStoreUnusable:
            // Not `restic_failed`: restic never ran. The store's own
            // refusal names the repair, so it is carried verbatim and the
            // code is non-retryable (#96).
            throw CLIFailure(
                code: .secretStoreUnusable,
                message: CLIFailure.bounded(
                    result.message ?? "The secret store could not be read."
                ),
                details: CLIErrorDetails(setId: setId, destinationId: destination.id)
            )
        }
