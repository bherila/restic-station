...
- **Global exclusions** — a built-in, platform-scoped list of paths never worth a snapshot (browser and system caches, build output, package-manager downloads, photo/video app previews, container storage, game installs) applies to every backup set out of the box, plus `restic backup --exclude-caches` for directories their own tools tagged `CACHEDIR.TAG` and an optional size cap. Groups are toggled per machine in **Settings → Exclusions** or with `restic-station excludes`, and any set can opt out entirely. It only ever affects new snapshots — nothing already backed up is removed.
