...
Reads recorded state (`state/schedule-state.json`, `state/current-run-*.json`, `state/repo-status-*.json`, `runs/index.jsonl`) and performs the same live locking probe as the app — no restic invocation. The lock probe may create the dedicated owner-only `locks/health.lock` inode so it can exercise `flock(2)` itself, and creates then removes a uniquely named scratch inode inside `locks/.health/` to catch quota or full-filesystem failures; it never creates operation locks. It inspects set locks only for sets in this machine's resolved configuration, so persistent orphaned names do not create false outages. `health` reuses `HealthDerivation.appHealth` verbatim (`Core/Sources/ResticStationCore/Support/HealthDerivation.swift`), so the CLI and the app's menu bar can never disagree about what counts as a warning.
