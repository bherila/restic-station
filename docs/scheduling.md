...
The operation-exclusion locks use `flock(2)` `LOCK_EX | LOCK_NB` on files under `locks/` (advisory; auto-released by the kernel on process death — no stale-lock cleanup needed; the files themselves persist, their existence means nothing). `AppPaths` creates every fresh internal lock-owning directory (`locks/`, `state/`, and `runs/`) through verified descriptors and pins it to `0700` after umask, so setup cannot create a tree the lock verifier then refuses. Each created or opened lock inode is likewise descriptor-pinned and verified at exact `0600`, including when a restrictive umask stripped owner bits at creation. Existing unsafe `locks/` or `runs/` directory modes are not silently repaired: live health still reports them. `state/` no longer re-tightens an existing mode either: a pre-existing `0755` is left in place, exactly as it is for `runs/` and `locks/`, while a pre-existing group/world-writable `state/` **refuses setup**. Note what that does and does not buy: `FileLock.verifyDirectory` rejects only group/world *write*, so a retained `0755` is not surfaced by live health either — it is accepted, not reported. That is deliberate rather than a gap to close, since a listable directory exposes nothing another uid can act on and the protection inside `state/` is per-file, but it does mean no diagnostic names it. A newly created `state/` is still pinned to `0700`. Not repairing cuts both ways, so a *restrictive* existing mode is refused too: `0500` or `0600` leaves the directory unusable — locks and temp files cannot be created — and is now reported with its own `chmod 700` guidance rather than silently accepted. (Which layer catches a mode without owner search is platform-dependent: Darwin's `O_SEARCH` needs execute so the open fails, while Linux's `O_PATH` opens regardless and the mode check refuses. Both refuse.) Automatic repair is what reintroduces the hazard — a refusal cannot be made atomic with a later `fchmodat`, so a directory that both refuses and repairs still has an interval in which a widen lands after the check and is erased by the chmod. Not repairing removes the interval instead of narrowing it; inside `state/` the protection that matters is per-file anyway. Setup runs long before the safety-authoritative schedule-state read (`Tick` calls `ensureDirectories()` roughly fifty lines earlier), so tightening that mode would erase the very evidence the read fails closed on and leave a healthy-looking tree after an exposure another uid could have written through. The refusal is made on the descriptor in hand rather than on a pathname pre-check, names the mode and the `chmod`, and deliberately leaves the mode in place until an operator acts. Because a scheduled tick exits at that call, it also carries the trusted-copy guidance the schedule-state reader would have given: an operator who only repairs the mode and restarts could otherwise admit a forged, checksum-valid watermark. `state/` is also the immediate lock parent for `schedule-state.lock`, so the schedule-state **read** path applies the same group/world-writable refusal to it that the lock verifier applies — one directory policy however `state/` is reached, rather than a weaker one on the path that publishes quarantine copies. Write bits only: a legacy `0755` `state/` still reads. Companion locks use the same primitive to serialize shared local files:
