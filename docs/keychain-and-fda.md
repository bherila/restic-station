...
A `RunAtLoad` tick can fire before the login keychain is unlocked; `find-generic-password` then fails (nonzero exit). This is **retryable** (see architecture.md error taxonomy): the engine must detect "password command failed" (restic exit 1 with keychain-ish stderr, or our own pre-flight `find-generic-password` probe failing) and simply skip the run WITHOUT writing a `.failed` record or updating `lastBackupStart` — the next tick (2 min later, keychain now unlocked) runs normally. Implement as a pre-flight in `BackupEngine.runSet`: read the primary's password via the active `SecretStore` first; on failure → log, exit, no state change. The same pre-flight covers the file backend's equivalent failures — an I/O error, an `EACCES` that Full Disk Access has not yet cleared — which is why `ResticRunnerError` calls the case `secretsUnavailable` rather than naming a backend. A **widened `secrets.json` is not one of them**: since #96 a mode the store refuses to read is `storeUnusable`, described below, because no repetition of the identical read performs the `chmod` its own refusal names. A destination with **nothing stored** (`SecretStoreError.itemNotFound` from either backend, and distinct from a *locked* keychain, which `security` reports as exit 51 rather than 44) is classified separately — `ResticRunnerError.secretsNotConfigured`, whose remedy is `secret set` and which the `--json` envelope reports as non-retryable. The engine's own pre-flight still skips it as retryable; see architecture.md §Error taxonomy and issue #95. A third condition — the store refusing to be read at all, because `secrets.json` is a symlink, is group- or world-accessible, is owned outside the trust boundary, or does not decode (the outer document or a stored secret-env blob, on either backend) — is `SecretStoreError.storeUnusable` and `ResticRunnerError.secretsStoreUnusable`, also non-retryable in the envelope (`secret_store_unusable`). The keychain's exit 51 stays retryable, which is what keeps the pre-login tick above working.
...
- `SecretStoreError.backendFailed` and `.storeUnusable` may only carry a backend's own diagnostic text (a path, an errno, a uid, a mode, a JSON key path) — never a value. `ResticRunnerError.secretsUnavailable`, `.secretsNotConfigured` and `.secretsStoreUnusable` carry only a destination id.
