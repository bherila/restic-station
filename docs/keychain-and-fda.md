...
- An **existing** `AppPaths.root` is **tightened to `0700` before a secret is written**, and `AppPaths.ensureDirectories()` creates a fresh one `0700` rather than `0755` — the two halves of issue #49. The directory is not defence in depth for a lucky ordering: whichever of the two code paths gets there first, a directory holding `secrets.json` is `0700` by the time the file exists. Consequences worth knowing, in both directions:
    ...
    - The **resulting** mode is checked with `stat(2)`, not `chmod`'s return value, because a filesystem can accept the call and ignore it. A group/world **write-and-search** mode is then fatal (another user could `rename(2)` a substituted `secrets.json` into place, which the `0600` file mode does nothing to stop); a merely readable or search-only mode (`0755`, `0711`) **warns and proceeds**, since the `0600` file still holds and refusing would stop backups over exposed metadata. Sticky directories (`S_ISVTX`, e.g. a data dir directly under `/tmp`) are exempt from the fatal case when both the directory's and the entry's owner are you or root — sticky mode already restricts replacement to those owners.
    - The **immediate parent** gets the same replacement check, one level only: a parent that lets another user `rename(2)` the data directory itself out from under us defeats every mode inside it. Ancestors above that are not audited.
    - All of the above runs on **write** paths only. `load()` and `print-password` are deliberately unaffected, so an existing host with a wide data directory keeps backing up on its 2-minute tick and finds out at the next `secret set` — the alternative fails backups outright on the whole upgrade cohort, which is a worse failure than the exposure it would be preventing.
