...
**Still deferred** — T26's live systemd `--user` timer end-to-end (`timer install` → two real firings → `journalctl` → `schedule-state.json` advanced): the `linux-integration` job's bare Ubuntu VM (unlike the `swift:6.1` container, this one really does boot with systemd as PID 1) makes an honest, non-blocking (`continue-on-error`) attempt at `timer install`/`timer status` as a diagnostic step, logging whatever `systemctl --user`/`loginctl` report — but a GitHub-hosted runner's default account still has no logind session, so this is expected to report the same "no systemd user bus" result the earlier deferral described, not clear it. Tracked as issue #45; verify on a real Linux host (or self-hosted runner with a real login session) to actually close this out.

### `docs/linux.md` transcripts (T30 / issue #32)

`scripts/linux-docs-transcript.sh` is not a test — nothing it does is asserted — it exists so that every command `docs/linux.md` quotes was actually run on Linux rather than paraphrased from a macOS terminal or invented. It runs against the `linux-integration` job's static binary and real restic (config import of the checked-in mac-exported-config.json fixture, secrets via stdin, repo init, a real backup + mirror copy, `status`/`runs`/`restore`, the per-machine override worked examples from `docs/data-model.md` re-validated against `Core/Tests/ResticStationCoreTests/Fixtures/config-v2.json`, and a `timer install`/`timer status` attempt on this same bare-VM host), echoing each command immediately before running it. Its job log is the source text for `docs/linux.md`'s transcripts — copied in by hand, not auto-inserted, but regression-*visible*: if a command's output changes, this step's log changes with it, and a docs update is due. What it deliberately does not attempt: anything needing a real logind session (`loginctl enable-linger`, a live-firing timer) — that gap is the same one issue #45 tracks, and `docs/linux.md` says so explicitly rather than showing invented output.
