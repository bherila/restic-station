...
Real restic (from PATH; CI: `brew install restic` on macOS, and the **official 0.18.1 release binary** on Linux — deliberately *not* `apt-get install restic`, because Ubuntu 24.04 ships 0.16.4, below the 0.17.0 minimum `ResticDiscovery` enforces, so any scenario that relies on discovery rather than a pinned `resticPath` fails with "restic not found"). Runs on **both platforms** (extended for Linux by T29 / issue #31 — one script, so the two platforms cannot silently drift in what they cover). Contract:
...
**Still deferred** — an `OnUnitActiveSec=` *repeat* firing over a longer time horizon, and the
timer surviving an actual logout of the account that ran `timer install`. As of T30,
`scripts/linux-docs-transcript.sh` (§next section) installs a timer for a dedicated, never-yet-run
backup set and polls for up to ~170s — long enough to observe a real, unattended **first** firing
(`OnBootSec=`, `"trigger":"scheduled"` confirmed in both `runs list --json` and `journalctl`), so
that specific gap this note used to describe is closed; the earlier assumption that a
GitHub-hosted runner's default account has no logind session at all also turned out to be wrong
for this runner (`ubuntu-24.04-arm`) — `loginctl` reports a real session with lingering already
enabled. What a single ~3-minute CI job still cannot show: a second, `OnUnitActiveSec=`-triggered
firing (the job doesn't wait that long), or survival across the job's own account actually logging
out (the job's own session never ends mid-run). Both remain genuinely open, tracked as issue #45
(updated with this finding and deliberately left open, not closed) — verify on a real Linux host
(or self-hosted runner with a real, ending login session) to close it out fully.

### `docs/linux.md` transcripts (T30 / issue #32)

`scripts/linux-docs-transcript.sh` is not a test — nothing it does is asserted — it exists so that every command `docs/linux.md` quotes was actually run on Linux rather than paraphrased from a macOS terminal or invented. It runs against the `linux-integration` job's static binary and real restic: config import of the checked-in mac-exported-config.json fixture, secrets via stdin, repo init, a real backup + mirror copy, `status`/`runs`/`restore`, the per-machine override worked examples from `docs/data-model.md` re-validated against `Core/Tests/ResticStationCoreTests/Fixtures/config-v2.json`, `timer install`/`timer status` on this same bare-VM host (including a direct demonstration that `timer install` bakes `RESTIC_STATION_DATA_DIR` into the unit but never `XDG_STATE_HOME`), a live monitoring-gap check (`status --json` staying exit 0 after the timer is uninstalled, contrasted with `timer status` correctly exiting 1) — and, installing a timer for a dedicated backup set that has never run before and polling for up to ~170s for it to fire on its own, which it does: a genuine, unattended `OnBootSec=` firing with `"trigger":"scheduled"` confirmed both in `runs list --json` and `journalctl`. Its job log is the source text for `docs/linux.md`'s transcripts — copied in by hand, not auto-inserted, but regression-*visible*: if a command's output changes, this step's log changes with it, and a docs update is due. What it still does not attempt: anything needing a real logind session ending mid-job (a timer surviving an actual logout) or an `OnUnitActiveSec=` repeat firing over a longer horizon than one job can wait for — that narrower remaining gap is tracked as issue #45 (updated, not closed, by this observation), and `docs/linux.md` says so explicitly rather than showing invented output.
