...
Real restic (from PATH; CI: `brew install restic` on macOS, `apt-get install restic` on Linux). Runs on **both platforms** (extended for Linux by T29 / issue #31 — one script, so the two platforms cannot silently drift in what they cover). Contract:
