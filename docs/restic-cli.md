...
`--exclude-caches`, `--exclude-larger-than` and the `--iexclude` block come from the global exclusion list (`docs/data-model.md` §global-excludes.json) — a built-in catalogue adjusted per host in `global-excludes.json`, supplied by `BackupSet.globalBackupExcludes(applying:)`. A set with `"usesGlobalExcludes": false` gets none of them, and a set with `"onlineOnlyFiles": "download"` gets all of them except the catalogue's cloud-placeholder patterns (`*.icloud`) — it has asked for the real contents of its evicted files, and the stub is the only record they exist (`docs/data-model.md` §Cloud placeholders).

**`--iexclude`, not `--exclude`, for the catalogue.** It is a list of well-known names rather than something a person typed, so `Library/Caches` must also skip `library/caches` and `*.dmg` must also skip `Installer.DMG`. A pattern the set already names on `--exclude` is **kept** in the `--iexclude` block rather than dropped as a duplicate: the two flags are different rules, so a set naming the common lowercase `node_modules` does not cover `NODE_MODULES`, and dropping the catalogue entry would leave those directories backed up with global exclusions on. The cost is one redundant match in the argv. The `--exclude` block *is* deduplicated, because there this host's `extraPatterns` and the set's own list ride the same flag. Neither flag is version-gated: restic has carried `--exclude-caches` since 0.9 and `--iexclude`/`--exclude-larger-than` since 0.13, unlike `--exclude-cloud-files` above.
...
Local-path destinations include external volumes (`/Volumes/...`) and cloud-sync-mounted folders (iCloud Drive under `~/Library/Mobile Documents/...`, plus OneDrive, SharePoint and other File Provider services under `~/Library/CloudStorage/...`). Cloud storage may evict repo pack files to dataless placeholders. Restic Station checks cloud-backed local repositories (detected the same way as sources) in the local reachability probe — which every probe-first command runs, so the refusal is published as `cloud_repository_not_hydrated` and badges **Error** — and again before every restic invocation — including the source repository of a `copy`, and the app's own `mount`, which does not go through the runner — and refuses to run when any entry is dataless, avoiding an implicit multi-gigabyte hydration. Both checks are scans taken before launch, not a guarantee about the running process: a file macOS evicts after the scan — including during a long `check`, `prune` or `copy`, or while browsing a mount — is still downloaded when restic reads it. Enforcing that on the restic process itself is tracked in #156. The destination editor surfaces the same warning and directs the user to make the repository available offline first.
