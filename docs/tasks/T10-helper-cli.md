...
- `RunSet.swift` — `run-set --set <uuid> [--kind backup|check|prune]` (default backup): loads config, finds set (unknown uuid → exit 1), calls the matching engine method with `.manual`. Set-lock busy → exit **2** (the app treats 2 as "already running").
  - **Exception — `--kind prune` is contained (#111/#82).** It refuses with `operation_not_allowed` and exit 1 as the *first statement of `run()`*, before the context is built — so no config load, no set resolution, no restic discovery or hashing, no secret read, no lock, and no run record. That ordering is deliberate and asserted by `ManualRetentionContainmentCLITests`: a prune request against invalid configuration or an unknown/disabled set returns the containment error rather than `config_invalid` or `set_not_found`, and that is correct, not a classification bug. The steps above apply again when #111 and #82 re-enable the operation.
