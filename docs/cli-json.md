...
is reachable only from the file backend today — not because the code asks which
backend answered, but because the keychain backend has no condition that meets
the test above.
