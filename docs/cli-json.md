...
| `cloud_repository_not_hydrated` | no | 1 | A local repository inside a cloud-synced folder (`~/Library/Mobile Documents`, `~/Library/CloudStorage`) has an online-only entry, so restic was not started — reading it would download repository data implicitly. Detected from file metadata before launch, never from restic's stderr. Make the repository available offline in the cloud provider, then retry. |
