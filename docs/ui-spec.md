...
**Set editor** (form):
- Name.
- **Sources**: list of absolute paths; add via `NSOpenPanel` (directories + files, multi-select), remove; warn inline (yellow) on nested/duplicate paths. A source under `~/Library/Mobile Documents` or `~/Library/CloudStorage` (symlinks resolved) is marked "Cloud-synced folder — follows the set's Online-only Files setting." (also in the machine-override sources editor).
- **Online-only files** (shown only while a source — shared or in any machine override — reaches cloud storage, including a source such as the home directory that contains a cloud folder): segmented picker *Skip* / *Download* bound to `onlineOnlyFiles`; the footer explains the selection — *Skip*: left out of snapshots instead of downloaded (restic 0.19 or newer), keep the folder available offline if every file must be backed up; *Download*: the provider downloads each online-only file first, snapshots are complete at the cost of the download and disk space.
- **Excludes**: "Apply global exclusions" toggle (`BackupSet.usesGlobalExcludes`, on by default — Settings → Exclusions is where the list itself lives), then an editable string list; caption linking restic exclude-pattern syntax (`https://restic.readthedocs.io/en/stable/040_backup.html#excluding-files`). Empty state with the toggle on: "Everything under the sources is backed up, apart from the global exclusions."; with it off: "Everything under the sources is backed up." Footer adds "This set also skips the global exclusions from Settings → Exclusions." or, when opted out, "This set ignores the global exclusions from Settings → Exclusions, including CACHEDIR.TAG directories. Only this set's own patterns are skipped — the ones above, plus anything listed under Purge."
...
- **Destinations**: table (label, repo, kind badge, PRIMARY tag, status dot reachable/offline/stale + "last synced N days ago"). Exactly-one-primary enforced by radio-style selection; changing primary shows an explanatory confirmation (new primary must already contain the data or the next backup re-uploads everything).
  - **Destination editor** (sheet): label; kind picker driving the form:
    ...
    - *Local folder*: path picker; inline warning if under `~/Library/Mobile Documents` or `~/Library/CloudStorage` ("Cloud storage may evict repository files to online-only placeholders. Restic Station will not use the repository while any file is online-only, rather than download it unexpectedly — keep this folder available offline in the cloud provider (for example “Always Keep on This Device” or “Keep Downloaded”). Sync folders replicate deletions — treat this as a convenience copy, not your only backup."); note when under `/Volumes` ("Removable volume — will be skipped when not mounted" — for secondaries this is normal).
...
> **Manual retention apply is contained.** `ManualRetentionApplyAvailability.isEnabled` is `false`, and the helper refuses `run-set --kind prune` before it builds a context — no config load, no restic hashing, no secret read, no lock, no run record, no subprocess. Core's `BackupEngine.runPrune` refuses independently for direct callers. The reason: the manual path proves mirror freshness from a persisted wall-clock comparison (#111) and forwards no exact keep/remove plan to the helper (#82), so it can apply a policy to repository state the operator never reviewed. Retention inside `runSet` has neither defect — it prunes a mirror only when *this run's* copy to it succeeded, and it orders secondaries before the primary — and stays enabled for every backup run, scheduled tick and hand-started Back Up Now alike (`runSet` applies retention regardless of trigger). **Preview cleanup remains available; it is read-only.** For a set whose backups succeed, the cost is timing, not capability: a behind mirror was already refused by the manual path's own freshness guard, and a caught-up one is cleaned after the next successful backup run's copy to it — scheduled, or Back Up Now. The genuine exception: retention only follows a *successful* backup, so a set whose backups fail (canonically, a full destination volume) receives no retention at all and there is no in-product way to free that space by dropping snapshots — recovery is a by-hand `restic forget --prune` carrying the set's configured `--keep-*` flags (dry-run first), deliberately, since a break-glass flag would bypass the containment. **Reclaim space** stays enabled and keeps the timestamp freshness gate as defense-in-depth (#111 residue): it removes only unreferenced packs, a different risk class from `forget`. Re-enabling is the joint acceptance criterion for #111 and #82.
...
- **Exclusions**: the global exclusion list this machine applies to every backup set that has not opted out (`docs/data-model.md` §global-excludes.json). Master toggle "Apply the global exclusion list"; "Skip directories tagged CACHEDIR.TAG"; "Skip files larger than" (a checkbox plus a size field, off by default, seeded at `10G`, showing "Not saved: use a number followed by k, m, g or t — for example 500m or 10G." while the typed value is incomplete); one toggle per built-in group with its title, one-sentence summary and the pattern count that applies on this platform; an editable string list of this machine's own extra patterns (caption linking the same restic exclude-pattern syntax the set editor does); and a footer showing the `global-excludes.json` path, the resolved pattern count, and "Restore Built-in Defaults". Footers: "These patterns are skipped by every backup set unless the set turns off “Apply global exclusions”. They only affect new snapshots — nothing already backed up is removed."; "CACHEDIR.TAG is a marker some tools (Cargo, Go) write into their own cache directories to say “do not back this up”."; "A size cap is off by default. Every other rule here names a folder of things that come back on their own; a size cap can skip one irreplaceable file — a video, a disk image — with nothing to point at afterwards."; "Turning a group off backs its paths up again from the next run. A newer version of Restic Station can add a group, and it applies unless you turn it off here."; "This file is not shared between machines and is never included in an exported configuration — a cache path belongs to a machine, not to a fleet." An unusable `global-excludes.json` shows a red banner above everything else, with: "Backups refuse to run until this is fixed, rather than falling back to the built-in defaults: the defaults may skip more than you had configured, which would quietly stop backing up directories you had kept. Restoring, unlocking and checking a repository are unaffected — this list only ever applies to a backup."
