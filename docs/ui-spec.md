...
**Set editor** (form):
- Name.
- **Sources**: list of absolute paths; add via `NSOpenPanel` (directories + files, multi-select), remove; warn inline (yellow) on nested/duplicate paths. A source under `~/Library/Mobile Documents` or `~/Library/CloudStorage` (symlinks resolved) is marked "Cloud-synced folder — see Online-only Files below."
- **Online-only files** (shown only while a source is cloud-synced): segmented picker *Skip* / *Download* bound to `onlineOnlyFiles`; the footer explains the selection — *Skip*: left out of snapshots instead of downloaded (restic 0.19 or newer), keep the folder available offline if every file must be backed up; *Download*: the provider downloads each online-only file first, snapshots are complete at the cost of the download and disk space.
