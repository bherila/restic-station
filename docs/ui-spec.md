...
**Set editor** (form):
- Name.
- **Sources**: list of absolute paths; add via `NSOpenPanel` (directories + files, multi-select), remove; warn inline (yellow) on nested/duplicate paths. Sources under `~/Library/Mobile Documents` or `~/Library/CloudStorage` explain that online-only files are skipped with `--exclude-cloud-files` (restic 0.19 or newer) instead of being downloaded, so snapshots contain only files stored on the Mac.
