...
public enum ManualRetentionApplyAvailability {
    ...
    /// The failed-backup exception and its recovery, as one sentence every
    /// surface composes rather than paraphrases — the caption, the tooltip,
    /// the agent-disabled branch, and the helper's stderr must not drift
