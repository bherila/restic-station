...
# config.json: every helper invocation below therefore runs the real v1 → v5
...
assert_migration() {
    local step="migration (v1 -> v5, resticPath relocated, v1 backed up)"
    log "$step"
    ...
    [[ "$version" == "5" ]] || fail "$step" "expected config.json at version 5, got '$version'"
